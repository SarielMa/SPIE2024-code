...
def parse_opt(known=False):
    ...
    opt.name = "TRADES_L2_"+str(opt.epsilon_train)
    return opt
...
class KLloss:
    ...
    def __call__(self, p, targets):
        # p shape should be a list of size 3 (model under train mode or the second element when eval mode)
        ret = 0
        n = len(p)
        for i in range(n):
            ret += self.loss(F.log_softmax(p[i].view(p[i].size(0), -1), dim=1),
                             F.softmax(targets[i].view(targets[i].size(0), -1), dim=1))
        return ret/n

def trades_loss(loss_fn, model,
                x_natural,
                y,
                optimizer,
                step_size,
    ...
    # define KL-loss
    criterion_kl = KLloss()
    model.eval()# when train mode, the output is list of size 3;
    # when eval mode, the output is a list [sth, list of size 3]
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss_kl = criterion_kl(model(x_adv)[1], model(x_natural)[1])# index 1 is because this is eval mode
        ...
        for _ in range(perturb_steps):
            ...
            loss = (-1) * criterion_kl(model(x_adv)[1], model(x_natural)[1])
            #loss.backward()
            # renorming gradient
            ...
            delta.data.clamp_(0.0, 1.0).sub_(x_natural)
    ...
    loss_natural, loss_items = loss_fn(logits, y)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                max_iter = 10
                step = noise/5
                loss, loss_items = trades_loss(loss_fn = compute_loss,
                                    model = model,
                                    x_natural = imgs ,
                                    y = targets.to(device),
                                    optimizer = optimizer,
