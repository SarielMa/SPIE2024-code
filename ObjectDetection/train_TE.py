...
from utils.dataloaders import my_create_dataloader, create_dataloader
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default=ROOT / 'runs/train_AT', help='save to project/name')
    ...
    parser.add_argument('--epsilon_train', type=int, default=2, help='noise level used for adversarial training')
    parser.add_argument('--beta', type=int, default=6.0, help='beta for training yolo')

    opt =  parser.parse_known_args()[0] if known else parser.parse_args()
    os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"] = str(opt.device)
    opt.name = "TE_Linf"+str(opt.epsilon_train)
    return opt
...
class PGD_TE:
    def __init__(self, loss_fn, num_samples=50000,  momentum=0.9, 
                 es=90, step_size=0.003, epsilon=0.031, perturb_steps=10, norm='linf'):
    ...
    def __call__(self, x_natural, y, index, epoch, model, optimizer, weight):
        ...
        if len(self.soft_labels) == 0:
            ...
            for i in range(n):
                self.soft_labels.append(torch.zeros(self.num_samples, 
                                                    logits[i].shape[1]*logits[i].shape[2]*logits[i].shape[3]*logits[i].shape[4], 
        ...
        if self.norm == 'linf':
            ...
            raise "L norm not supported"
            
        for _ in range(self.perturb_steps):
            x_adv.requires_grad_()
            with torch.enable_grad():
                logits_adv = model(x_adv)[1]
                if epoch >= self.es:
                    loss = self.loss_fn(logits_adv, y)[0] + weight * self.multipleOutputReg(logits_adv, soft_labels_batch)
                else:
                    loss = self.loss_fn(logits_adv, y)[0]
        ...
        model.train()
        optimizer.zero_grad()
        x_adv2 = Variable(torch.clamp(x_adv, 0.0, 1.0), requires_grad=False)     
        # calculate robust loss
        logits = model(x_adv2)
        loss, lossitems = self.loss_fn(logits, y)
        if epoch >= self.es:
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_loader, dataset = my_create_dataloader(train_path,
                                              imgsz,
    ...
    #-------------te
    es_start = opt.epochs * 0.5 # should be 0.5
    es_end = opt.epochs * 0.7
    noise = opt.epsilon_train/255
    pgd_te = PGD_TE(  loss_fn = compute_loss,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, idx) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                #-----te loss
                loss, loss_items = pgd_te(imgs, targets.to(device), idx, epoch, model, optimizer, weight)
                #-----te loss end
                if RANK != -1:
